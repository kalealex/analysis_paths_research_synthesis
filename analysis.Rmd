...
```{r echo=FALSE}
ggplot(d.strat, aes(x=Strategy,y=Frequency)) + geom_bar(stat = 'identity') + geom_text(aes(label=Frequency),position=position_dodge(width=0.9), size=4, vjust=-0.35) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black"))
```
...
```{r echo=FALSE}
ggplot(d.strat.mention, aes(x=Strategy,y=Frequency)) + 
  geom_bar(stat = 'identity') + 
  geom_text(aes(label=Frequency),position=position_dodge(width=0.9), size=4, vjust=-0.35) + 
...
```{r echo=FALSE}
ggplot(d.strat.impact, aes(x=Strategy,y=Frequency)) + 
  geom_bar(stat = 'identity') + 
  geom_text(aes(label=Frequency),position=position_dodge(width=0.9), size=4, vjust=-0.35) + 
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  facet_grid(~ Impact)
...
```{r echo=FALSE}
ggplot(d.strat.stage, aes(x=Strategy,y=Frequency)) + 
  geom_bar(stat = 'identity') + 
  geom_text(aes(label=Frequency),position=position_dodge(width=0.9), size=4, vjust=-0.35) + 
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  facet_wrap(~ Stage)
...
codes$Motivation <- with(codes, 
                         ifelse(Reason %in% c("Consistency", "Reproducibility", "Completeness"), "Principles",
                            ifelse(Reason %in% c("Confidence through concensus", "Ease of understanding"), "Social Factors",
                               ifelse(Reason %in% c("Domain knowledge", "Conceptual clarity"), "Domain-Specific Factors",
                                  ifelse(Reason %in% c("Statistical power", "Convention"), "Conventions",
...
```{r echo=FALSE}
ggplot(codes, aes(x=Motivation,fill=Reason)) + 
  geom_bar(stat = 'count') + 
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"))
```
...
A similarly prevalent reason for decision-making is completeness, the idea that a researcher should cover their bases and put together a comprehensive analysis. The drive for completeness contributes heavily to the high frequency decisions motivated by principles.
...
Conventions are also a common motivation for analytical decisions. To the extent that conventions represent ideal ways of conducting research, this is probably a good thing. However, in cases where conventions reflect the calcified practices of research communities and not best practices, appeals to convention can be no more justified than instances of suppression.
...
While the sixe motivational themes are somewhat balanced in their overall prevalence, we can see that certain motivations more often correspond with different kinds of decision-making strategies. 
